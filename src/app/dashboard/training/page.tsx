  ...
  return (
    ...
    >
      <Card className="overflow-hidden bg-card border-white/10">
        {/* Day Header */}
        <CardHeader className="bg-gradient-to-r from-emerald-950/50 to-transparent">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-base text-foreground">{day.day_name}</CardTitle>
                <CardDescription>
                  {day.workout_name}
        ...
        <CardContent className="space-y-5">
          ...
          {warmupSteps.length > 0 && (
            <div>
              ...
              <div className="rounded-lg border border-orange-500/20 bg-orange-900/20 p-3">
                <ul className="space-y-1.5">
                  {warmupSteps.map((step, i) => (
                    ...
                    >
                      <span className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-orange-900/40 text-[10px] font-bold text-orange-400">
                        {i + 1}
                      </span>
                      {step}
                    </li>
          ...
          {exercises.length > 0 && (
            <div>
              ...
              <div className="rounded-lg border border-white/10 overflow-hidden">
                <Table>
                  <TableHeader>
                    <TableRow className="bg-muted hover:bg-muted border-white/10">
                  ...
                  <TableBody>
                    {exercises.map((ex, i) => (
                      <TableRow key={i} className="border-white/10 odd:bg-card even:bg-zinc-950 hover:bg-zinc-800">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              {ex.intensity_technique && ex.intensity_technique !== 'none' && (
                                <Badge variant="outline" className="ml-1 text-[10px] px-1 py-0 border-white/10">
                        ...
                        <TableCell className="text-center">
                          {ex.rpe ? (
                            <Badge
                              variant="secondary"
                              className="gap-0.5 text-xs bg-zinc-800"
                            >
                        ...
                        <TableCell className="text-center">
                          {ex.rir !== null && ex.rir !== undefined ? (
                            <span className="text-xs text-foreground">{ex.rir}</span>
                          ) : (
                            <span className="text-xs text-muted-foreground">
                              --
                            </span>
...
export default function TrainingPage() {
      ...
      <div className="flex h-full items-center justify-center p-6">
        <Card className="max-w-md bg-card">
          <CardContent className="flex flex-col items-center gap-4 pt-6 text-center">
            ...
            <div>
              <p className="font-semibold text-foreground">Failed to load training plan</p>
  ...
  if (trainingData.length === 0) {
    return (
      <div className="flex h-full items-center justify-center p-6">
        <Card className="max-w-md bg-card border-white/10">
          <CardContent className="flex flex-col items-center gap-4 py-16 text-center">
            <div className="rounded-full bg-emerald-500/10 p-4">
              <Dumbbell className="h-10 w-10 text-emerald-400/60" />
            </div>
            <div>
              <p className="text-lg font-semibold text-foreground">No training plan yet</p>
    ...
    <div className="p-6 lg:p-8">
      ...
      {progressionRules && (
        <motion.div
          ...
          className="mb-6"
        >
          <Card className="border-violet-500/20 bg-violet-900/20">
            <CardContent className="pt-6">
              <div className="flex gap-3">
                <ChevronUp className="mt-0.5 h-5 w-5 shrink-0 text-violet-400" />
                <div>
                  <p className="font-semibold text-violet-400">
                    Progression Rules
                  </p>
                  <p className="mt-1 text-sm leading-relaxed text-violet-400/70">
                    {progressionRules}
                  </p>
                </div>
              </div>
