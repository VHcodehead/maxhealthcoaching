// POST /api/coach/push/cardio — coach-gated proxy to the write-back bridge.
import { NextRequest, NextResponse } from 'next/server';
import { auth } from '@/lib/auth';
import { prisma } from '@/lib/db';
import { pushCardio, CoachingBridgeError } from '@/lib/coaching-bridge';
...
export async function POST(request: NextRequest) {
  ...
  try {
    const data = await pushCardio(body);
